...
const totalCasesChart = Highcharts.chart('total-cases-chart-container', {
  ...
  tooltip: {
    ...
    formatter: function() {
      ...
      text += `<div class="tooltip-name">${timestamp2DateStr(this.points[0].x)}</div></div>`;
      text += '<div class="tooltip-info-column">Đang mắc:&nbsp;<br>Đã khỏi:&nbsp;<br>Tử vong:&nbsp;</div>';
      text += `<div class="tooltip-info-column">${addSeparator(this.points[0].y, ' ')}<br>`;
      text += `${addSeparator(this.points[1].y, ' ')}<br>${addSeparator(this.points[2].y, ' ')}</div>`;     
      text += '</div>';
      return text;
    },
  }
