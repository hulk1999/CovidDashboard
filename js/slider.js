...
function handleSliderInput(el, isLeft, isUpdate) {
  ...
  // update charts
  for (let i = 0; i < 3; i++){
    casesCharts[i].series[0].setData(chInfos[i].data.DailyCases.slice(self.left, self.right+1), redraw=false);
    casesCharts[i].series[1].setData(chInfos[i].data.DailyCases7DaysAvg.slice(self.left, self.right+1), redraw=false);
    casesCharts[i].redraw();
  }
  totalCasesChart.series[0].setData(casesData.CurrentlyInfected.slice(self.left, self.right+1), redraw=false);
  totalCasesChart.series[1].setData(casesData.Recoveries.slice(self.left, self.right+1), redraw=false);
  totalCasesChart.series[2].setData(casesData.Deaths.slice(self.left, self.right+1), redraw=false);
  totalCasesChart.redraw();
  if (isPeople){
    vaccineChart.series[0].setData(vaccineData.partialPeople.slice(self.left, self.right+1), redraw=false);
    vaccineChart.series[1].setData(vaccineData.fullPeople.slice(self.left, self.right+1), redraw=false);
  } else{
    vaccineChart.series[0].setData(vaccineData.partialPct.slice(self.left, self.right+1), redraw=false);
    vaccineChart.series[1].setData(vaccineData.fullPct.slice(self.left, self.right+1), redraw=false);
  }
  vaccineChart.redraw();
